...
- The individual 31 skills contain the extraction/generation logic. This orchestrator coordinates them. This includes the 6 conversational-experience attributes for Google's AI Mode (`question_and_answer`, `document_link`, `related_product`, `item_group_title`, `variant_option`, `popularity_rank`).
...
### Category 4: Conversational-experience attributes (Google AI Mode) — mixed behavior
These 6 attributes power Google's conversational/AI Mode shopping experience. They don't fit the extract/generate/classify buckets cleanly — some are group attributes, some are cross-product, some require merchant performance data. Each has its own skill; the orchestrator routes them in the phases below.

| Attribute | Behavior | Language | Notes |
|---|---|---|---|
| `question_and_answer` | Generative (conservative) | matches feed `[output_lang]` | Group attr (question+answer), ≤30 pairs, 10k char cap, Sheets-escaped. Only from hard facts. |
| `item_group_title` | Derived from variant group | matches feed (derived from titles) | One shared title per `item_group_id`, stripped of variant tokens. Needs Phase 3 `item_group_id`. |
| `variant_option` | Extraction (group attr) | values verbatim from feed | `name:value` pairs of varying dimensions. Needs `item_group_id`. Same name-set per group. |
| `related_product` | Cross-product heuristic | n/a (identifiers only) | Group attr (relationship_type+identifier_type+identifier). Prefer explicit feed columns. |
| `document_link` | Map-and-validate | n/a (URLs) | Never invents URLs. Detect PDF-URL column, validate format. |
| `popularity_rank` | Map / validate / compute | n/a (number) | Never invents. Validate existing, or compute percentile from a sales/quantity column. |
...
COLUMN_CANDIDATES = {
    ...
    # ── Conversational-experience attributes (Google AI Mode) ──
    'question_and_answer': ['question_and_answer', 'g:question_and_answer', 'q&a', 'faq'],
    'document_link':    ['document_link', 'g:document_link', 'document link', 'manual_url', 'manual', 'handleiding', 'datasheet', 'pdf_url', 'user_guide'],
    'related_product':  ['related_product', 'g:related_product', 'related products', 'gerelateerde producten'],
    'item_group_title': ['item_group_title', 'g:item_group_title', 'groepstitel', 'item group title'],
    'variant_option':   ['variant_option', 'g:variant_option', 'variant options', 'variantopties'],
    'popularity_rank':  ['popularity_rank', 'g:popularity_rank', 'populariteit', 'sales_rank', 'units_sold', 'quantity_sold', 'verkocht', 'bestseller_rank'],
}
...
Phase 5 — CONVERSATIONAL EXPERIENCE (6 skills, Google AI Mode)
  Run after Phase 4 so generative attributes (title/description/detail/highlight) exist
  and after Phase 3 so item_group_id exists.
  ├─ item_group_title  ← needs: item_group_id + variant titles   [output_lang]  ← derive shared group title
  ├─ variant_option    ← needs: item_group_id + color/size/...   ← extract varying dimensions per variant
  ├─ question_and_answer ← hard facts; must NOT duplicate title/description/detail/highlight  [output_lang]
  ├─ related_product   ← cross-product: explicit columns > heuristics (substitute/different_brand)
  ├─ document_link     ← map-and-validate PDF-URL column (never invents)
  └─ popularity_rank   ← validate existing OR compute percentile from sales/quantity (never invents)
```
...
# Phase 5: conversational-experience attributes
# item_group_title and variant_option REQUIRE item_group_id (from Phase 3 or the feed).
#   If no item_group_id exists, skip both and report; do not fabricate groups.
# question_and_answer must be wired with the ALREADY-GENERATED title/description/
#   product_detail/product_highlight so it can avoid duplicating them (Google rule).
# related_product / document_link / popularity_rank are cross-product or map-and-validate:
#   they read whole-feed structures or dedicated source columns, not single-row text.
#   When their required source data is absent, they report "not possible" rather than guess.
group_title_value = derive_item_group_title(group_df, lang=output_lang)   # per item_group_id
variant_option_value = extract_variant_option(group_df)                   # per item_group_id
qa_value = generate_qa(hard_facts, lang=output_lang,
                       exclude=[title_value, description_value, detail_value, highlights_value])
```
...
OUTPUT_COLUMNS = [
    ...
    # Phase 5: Conversational experience (Google AI Mode)
...
After all 31 skills have run and the supplemental feed is assembled, run these cross-attribute sanity checks. These are pure logical checks on the output — language-independent, vertical-independent. They catch contradictions between attributes that no individual skill can detect on its own.
...
for idx, row in supplemental.iterrows():
    ...
    # ── CHECK 8: item_group_title must differ from variant title ──
    # Google rejects an item_group_title equal to the variant's own title.
    igt = str(row.get('item_group_title', '')).strip()
    vtitle = str(row.get('title', '')).strip()
    if igt and vtitle and igt.lower() == vtitle.lower():
        supplemental.at[idx, 'item_group_title'] = ''
        validation_flags.append((product_id, 'igt_equals_title', "item_group_title equals variant title — cleared"))

    # ── CHECK 9: variant_option / item_group_title require item_group_id ──
    # Both only apply to grouped variants. If the product has no item_group_id, clear them.
    igid_val = str(row.get('item_group_id', '')).strip()
    if not igid_val or igid_val.lower() in ('nan', 'none'):
        if str(row.get('variant_option', '')).strip():
            supplemental.at[idx, 'variant_option'] = ''
        if str(row.get('item_group_title', '')).strip():
            supplemental.at[idx, 'item_group_title'] = ''

    # ── CHECK 10: popularity_rank range and format ──
    # Must be a number 0-100 with at most 1 decimal, no % sign.
    pr = str(row.get('popularity_rank', '')).strip().replace('%', '')
    if pr and pr.lower() not in ('nan', 'none'):
        try:
            v = round(min(100.0, max(0.0, float(pr.replace(',', '.')))), 1)
            supplemental.at[idx, 'popularity_rank'] = f"{v:.1f}"
        except ValueError:
            supplemental.at[idx, 'popularity_rank'] = ''
            validation_flags.append((product_id, 'popularity_rank_invalid', f"non-numeric popularity_rank '{pr}' — cleared"))

    # ── CHECK 11: document_link must be a .pdf http(s) URL ──
    dl = str(row.get('document_link', '')).strip()
    if dl and dl.lower() not in ('nan', 'none'):
        urls = [u.strip() for u in dl.split(',') if u.strip()]
        kept = [u for u in urls if re.match(r'^https?://', u, re.I)
                and u.split('?', 1)[0].lower().endswith('.pdf') and u.isascii()]
        if len(kept) != len(urls):
            supplemental.at[idx, 'document_link'] = ', '.join(kept)
            validation_flags.append((product_id, 'document_link_invalid', "dropped non-PDF/non-http/non-ASCII document URLs"))
...
| 8 | item_group_title equals variant title | Auto-clear + flag | Google rejects a group title identical to the variant title |
| 9 | variant_option / item_group_title without item_group_id | Auto-clear | Both only apply to grouped variants |
| 10 | popularity_rank non-numeric or out of range | Clamp/round, or clear + flag | Must be 0–100, max 1 decimal, no % |
| 11 | document_link not a PDF/http(s)/ASCII URL | Drop bad URLs + flag | Google requires public, RFC-3986, .pdf links |
...
══════════════════════════════════════════════════════
  ...
  Phase 5 (AI Mode):        6 skills (Q&A/variants/related/docs/popularity)
...
2. **Language detection supports 7 languages.** NL, EN, DE, FR, IT, ES, PT. It determines which output vocabulary/templates Phase 1 (color, material), Phase 4 (generative), and the language-bearing Phase 5 skills (question_and_answer, item_group_title) use. Other skills ignore it.
3. **Column mapping is flexible.** Handles NL, EN, DE, FR, g:-prefixed, and vendor-specific column names.
4. **Execution order is strict.** Phase 1 → 2 → 3 → 4 → 5. Dependencies flow downward. Phase 5 (AI Mode) runs last: it needs item_group_id from Phase 3 and the generated text from Phase 4.
...
12. **AI Mode attributes never fabricate.** `document_link` and `popularity_rank` map-and-validate (or compute popularity from a sales column) and report "not possible" when the source data is absent. `related_product` prefers explicit feed columns over heuristics. `question_and_answer` generates only from hard facts and must not duplicate title/description/product_detail/product_highlight. `item_group_title` and `variant_option` only apply to grouped variants (require item_group_id).

## Guardrails inherited from skills

The orchestrator inherits all guardrails from the 31 individual skills:

- **Conversational-experience (AI Mode) attributes never invent data.** `document_link` and `popularity_rank` only map/validate/compute from real source columns and otherwise report "not possible". `related_product` only links ids/GTINs that exist in the feed, treats same-item_group_id products as variants (not sets), and prefers explicit relationship columns over heuristics. `question_and_answer` draws only from hard feed facts, respects the 30-pair / 1000-char-per-field / 10,000-char-total limits, and avoids duplicating other attributes. `item_group_title` and `variant_option` only apply to 2+ variant groups, keep a consistent name-set per group, and use Google Sheets escaping for reserved characters (`:`, `,`, `\`).

- **Never overwrite good data.** Nearly all skills skip products with existing values. Only `title` (score < 60) and `description` (< 150 chars) may modify existing values; the AI Mode skills also respect existing values and only `popularity_rank` recomputes when the user explicitly asks.
